...
const Genres = ({
  ...
  const fetchGenres = async () => {
    try {
      const response = await fetch(
        `https://api.themoviedb.org/3/genre/${type}/list?api_key=b3eed8b2480412d418e8776c212425e9&language=en-US`
      );
  ...
  genres &&
    genres.map((genre) =>
      options.push({ value: genre.name, label: genre.name, id: genre.id })
    );
